...
class MinitaurBulletCarryEnv(gym.Env):
  ...
  def __init__(
    ...
    # add
    cwd_name = 'temp'
    rewards_dir = '/home/tetsuhara/minitaur_bullet/' + cwd_name + '/rewards'
    # rewards_dir = '/home/ytetsuhara/minitaur_bullet/' + cwd_name + '/rewards'
    self.forward_path = os.path.join(rewards_dir, 'forward_reward.txt')
    self.energy_path = os.path.join(rewards_dir, 'energy_reward.txt')
    self.roll_path = os.path.join(rewards_dir, 'roll_reward.txt')
    self.pitch_path = os.path.join(rewards_dir, 'pitch_reward.txt')
    #####
  ...
  def step(self, action):
    ...
    # add
    with open(self.forward_path, 'a') as f:
      f.write(str(reward[1]) + ',')
    with open(self.energy_path, 'a') as f:
      f.write(str(reward[2]) + ',')
    with open(self.roll_path, 'a') as f:
      f.write(str(reward[3]) + ',')
    with open(self.pitch_path, 'a') as f:
      f.write(str(reward[4]) + ',')
    #####
    # return np.array(self._noisy_observation()), reward, done, {}
    return np.array(self._noisy_observation()), reward[0], done, {}
  ...
  def _reward(self):
    ...
    # add
    orientation = self.minitaur.GetBaseOrientation()
    current_base_rpy = self._pybullet_client.getEulerFromQuaternion(orientation)
    roll_reward = -abs(current_base_rpy[0])
    pitch_reward = -abs(current_base_rpy[1])
    self._last_base_rpy = current_base_rpy
    #####
    ...
    #          self._drift_weight * drift_reward + self._shake_weight * shake_reward)
              self._drift_weight * drift_reward + self._shake_weight * shake_reward + 
              self._roll_weight * roll_reward + self._pitch_weight * pitch_reward)          
    # self._objectives.append([forward_reward, energy_reward, drift_reward, shake_reward])
    self._objectives.append([forward_reward, energy_reward, drift_reward, shake_reward,
                             roll_reward, pitch_reward])
    # add
    reward_list = [reward, forward_reward, energy_reward, roll_reward, pitch_reward]
    return reward_list
...
class MinitaurBulletCarry2Env(gym.Env):
  """The gym environment for the minitaur.
  ...
  def _reward(self):
    ...
    #print('rot_rad:{}, {}'.format(roll_reward, pitch_reward))
    #####
    reward = (self._distance_weight * forward_reward - self._energy_weight * energy_reward +
    #          self._drift_weight * drift_reward + self._shake_weight * shake_reward)
              self._drift_weight * drift_reward + self._shake_weight * shake_reward + 
